...
/**
 * LomoPlayerVTable:
 * @set_uri: Function for create/destroy/update pipelines.
...
static gboolean
player_set_stream(LomoPlayer *self, LomoStream *stream, GError **error);
static void
...
lomo_player_dispose (GObject *object)
{
	...
	if (priv->pipeline)
	{
		...
		player_set_stream(self, NULL, NULL);
		priv->pipeline = NULL;
	}
...
lomo_player_play_stream(LomoPlayer *self, LomoStream *stream, GError **error)
{
	...
	return (player_set_stream(self, stream, error) && lomo_player_play(self, error));
}
...
player_set_stream(LomoPlayer *self, LomoStream *stream, GError **error)
{
	...
	GstElement *new_pipeline = self->priv->vtable.set_uri(
		self->priv->pipeline, 
		stream ? lomo_stream_get_tag(stream, LOMO_TAG_URI) : NULL,
		self->priv->options);

	if (stream && !new_pipeline)
	{
	...
	if (new_pipeline != self->priv->pipeline)
	{
		if (self->priv->pipeline)
			g_object_unref(self->priv->pipeline);

		self->priv->pipeline = new_pipeline;
...
lomo_player_insert_multi(LomoPlayer *self, GList *streams, gint pos)
{
	...
	while (l)
	{
		...
		if (emit_change && !player_run_hooks(self, LOMO_PLAYER_HOOK_CHANGE, NULL, -1, 0))
		{
			...
			if (!player_set_stream(self, stream, &err))
			{
...
lomo_player_go_nth(LomoPlayer *self, gint position, GError **error)
{
	...
	// Create new pipeline
	if (!player_set_stream(self, (LomoStream *) stream, &suberror))
	{
